...
class Brp_model extends CI_Model
{
  ...
  function get_all_transaksi_individu()
  {
    ...
    $this->db->where('transaksi_individu.id_program', '6');
    $query = $this->db->get();
    return $query->row();
  }
  ...
  {
    $this->db->select('*,transaksi_individu.created_at as datecreated');
    $this->db->from('transaksi_individu');
    $this->db->join('individu', 'individu.nik = transaksi_individu.nik', 'left');
  ...
  function get_detail_komunitas_by_id($id)
  {
    ...
    $this->db->where('transaksi_komunitas.id_program', '6');
  ...
  {
    $this->db->select('*,
    transaksi_komunitas.nik as nik_transaksi,
    penduduk.nama as nama_penduduk,
    transaksi_komunitas.created_at as tanggal_transaksi,
    transaksi_komunitas.jenis_bantuan as jenis_bantuan,
    komunitas.id_provinsi as id_provinsi_komunitas,
    komunitas.id_kota_kab as id_kota_kab_komunitas,
    komunitas.id_kecamatan as id_kecamatan_komunitas,
    komunitas.id_desa_kelurahan as id_desa_kelurahan_komunitas,
    komunitas.alamat as alamat_komunitas,
    komunitas.kodepos as kodepos_komunitas,
    komunitas.negara as negara_komunitas,
    transaksi_komunitas_d.nama_det as nama_pasien,
    transaksi_komunitas_d.jenis_penyakit as jenis_penyakit,
    transaksi_komunitas_d.kelurahan as kelurahan_pasien,
    transaksi_komunitas_d.kecamatan as kecamatan_pasien,
    transaksi_komunitas_d.kota as kota_pasien,
    transaksi_komunitas_d.kesan as kesan_pasien
    ');
    $this->db->from('transaksi_komunitas');
  ...
  function get_data_komunitas_by_program()
  {
    $sql = "SELECT
            ...
            WHERE `b`.`id_kategori_komunitas` = 1
            ";
  ...
  function get_laporan_komunitas_detail($id, $total_periode = null)
  {
    $this->db->select('*,transaksi_komunitas.nik as nik_transaksi,penduduk.nama as nama_penduduk, transaksi_komunitas.created_at as tanggal_transaksi, komunitas.id_provinsi as id_provinsi_komunitas, komunitas.id_kota_kab as id_kota_kab_komunitas, komunitas.id_kecamatan as id_kecamatan_komunitas, komunitas.id_desa_kelurahan as id_desa_kelurahan_komunitas, komunitas.alamat as alamat_komunitas, komunitas.kodepos as kodepos_komunitas, komunitas.negara as negara_komunitas');
    $this->db->from('transaksi_komunitas');
    $this->db->join('rekomender', 'rekomender.id_rekomender = transaksi_komunitas.info_bantuan', 'left');
    ...
    $this->db->where('transaksi_komunitas.is_delete', '0');
    $this->db->where('transaksi_komunitas.nik', $id);
    $this->db->where('transaksi_komunitas.total_periode', $total_periode);
    $query = $this->db->get();
    return $query->result();
  }
}
