...
class Shelter_model extends CI_Model
{
  ...
  function get_all_transaksi_individu()
  {
    ...
    $this->db->where('transaksi_individu.id_program', '4');
    $query = $this->db->get();
    return $query->row();
  }
  ...
  function get_detail_komunitas_by_id($id)
  {
    ...
    $this->db->where('transaksi_komunitas.id_program', '4');
  ...
  {
    $this->db->select('*,pen1.nama as pen1_nama,pen1.jk as pen1_jk, pen2.nama as pen2_nama,pen2.jk as pen2_jk');
    $this->db->from('transaksi_individu');
    $this->db->join('penduduk', 'penduduk.nik = transaksi_individu.nik', 'left');
    $this->db->join('penduduk as pen1', 'pen1.id_penduduk = transaksi_individu.pendamping1', 'left');
    $this->db->join('penduduk as pen2', 'pen2.id_penduduk = transaksi_individu.pendamping2', 'left');
  ...
  function get_all_deleted_individu()
  {
    ...
    $this->db->join('individu', 'individu.nik = transaksi_individu.nik', 'left');
    // $this->db->join('penduduk', 'penduduk.id_penduduk = transaksi_individu.pendamping1');
    $this->db->join('penduduk', 'penduduk.nik = transaksi_individu.nik', 'left');
    $this->db->join('provinsi', 'penduduk.id_provinsi = provinsi.id_provinsi', 'left');
    $this->db->join('kecamatan', 'penduduk.id_kecamatan = kecamatan.id_kecamatan', 'left');
    $this->db->join('kota_kab', 'penduduk.id_kota_kab = kota_kab.id_kota_kab', 'left');
