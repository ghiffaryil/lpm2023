...
class Noreg extends CI_Controller
{
    ...
    function laporan_admin()
    {
        ...
        $sql_data_individu_bawah = "SELECT `a`.`id_transaksi_individu`, 
        `a`.`asnaf`, 
        `a`.`periode_bantuan`, 
        `c`.`nik`, `c`.`nama`, `c`.`nama_kk`, `c`.`jk`, `c`.`alamat`,
        `c`.`pekerjaan`,
        
         `e`.`desa_kelurahan`, `f`.`kecamatan_name`, `g`.`kota_kab`, `h`.`provinsi`, `c`.`no_hp`
                                FROM `transaksi_individu` `a`
                                JOIN `transaksi_individu_d` `b`
                                ON `a`.`id_transaksi_individu` = `b`.`id_transaksi_individu`
                                JOIN `penduduk` `c`
                                ...
                                JOIN `program` `d`
                                ON `a`.`id_program` = `d`.`id_program`
                                JOIN `desa_kelurahan` `e`
                                ON `c`.`id_desa_kelurahan` = `e`.`id_desa_kelurahan`
                                JOIN `kecamatan` `f`
                                ON `c`.`id_kecamatan` = `f`.`id_kecamatan`
                                JOIN `kota_kab` `g`
                                ON `c`.`id_kota_kab` = `g`.`id_kota_kab`
                                JOIN `provinsi` `h`
                                ON `c`.`id_provinsi` = `h`.`id_provinsi`
                                WHERE `a`.`id_program` = 10
                                AND `a`.`is_delete` = 0
                                GROUP BY `c`.`nik`";
        $res_data_individu_bawah = $this->db->query($sql_data_individu_bawah)->result();
        // echo json_encode($res_data_individu_bawah);
        // die;
        $this->data['individu_bawah']  = $res_data_individu_bawah;
    ...
    function laporan_pusat()
    {
        ...
        $sql_data_individu_bawah = "SELECT
         `a`.`id_transaksi_individu`,
         `a`.`asnaf`, 
         `a`.`periode_bantuan`, 
          `c`.`nik`, `c`.`nama`, `c`.`nama_kk`, `c`.`jk`, `c`.`alamat`, 
          `c`.`pekerjaan`, 
